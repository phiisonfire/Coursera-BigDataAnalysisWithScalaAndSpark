...
object TimeUsage extends TimeUsageInterface:
  ...
  def row(line: List[String]): Row = {
    val firstCol = line.head // first column is a String
    val restCol = line.tail.map(_.toDouble) // convert the rest to Double
    Row.fromSeq(firstCol :: restCol)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primaryNeedPrefixes = List("t01", "t03", "t11", "t1801", "t1803")
    val workingActivitiyPrefixes = List("t05", "t1805")
    val otherActivityPrefixes = List(
      "t02", "t04", "t06", "t07", "t08", "t09",
      "t10", "t12", "t13", "t14", "t15", "t16", "t18"
    )

    columnNames.foldLeft((List[Column](), List[Column](), List[Column]())){ (accTuple, columnName) => {
      if (primaryNeedPrefixes.exists(columnName.startsWith)) {
        (accTuple._1 :+ new Column(columnName), accTuple._2, accTuple._3)
      }
      else if (workingActivitiyPrefixes.exists(columnName.startsWith)) {
        (accTuple._1, accTuple._2 :+ new Column(columnName), accTuple._3)
      }
      else if (otherActivityPrefixes.exists(columnName.startsWith)) {
        (accTuple._1, accTuple._2, accTuple._3  :+ new Column(columnName))
      }
      else (accTuple._1, accTuple._2, accTuple._3)
    }}
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column =
      (primaryNeedsColumns.reduceLeft(_+_) / 60).as("primaryNeeds")
    val workProjection: Column =
      (workColumns.reduceLeft(_+_) / 60).as("work")
    val otherProjection: Column =
      (otherColumns.reduceLeft(_+_) / 60).as("other")
    df
